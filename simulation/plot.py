...
fig0_name_null = "results_data/K1000N5000c5key23333T1000Delta0.1-alpha0.05-max-null"
fig2_name = "results_data/K1000N5000c5key23333T700Delta0.5-alpha0.05-max-result"

fig0_name1_null = f"results_data/K1000N5000c5key23333T1000Delta0.1-alpha0.05-inv-null"
fig2_name1 = f"results_data/K1000N5000c5key23333T700Delta0.5-alpha0.05-inv-result"
...
ax[0][0].set_ylim(ymax=0.095,ymin=0.04)
if use_log:
...
for j, algo in enumerate(["ars", "log", "ind-1/e", "opt-001", "opt-0005"]):
    ...
    ax[0][1].plot(np.arange(1,1+first), mean[:first], label=labelize(algo), linestyle=linestyles[j%len(linestyles)],color=colors[j%len(colors)])

ax[0][1].set_title(r"$H_1, \Delta \sim {U}(0.001, 0.5)$")
...
first = 700
name = fig0_name1_null
save_dict = json.load(open(name+".json", "r"))
for j, algo in enumerate(["dif", "dif-opt-01", "dif-opt-001", "dif-opt-0001"]):
    mean = np.array(save_dict[algo])[:first]
    ax[1][0].plot(np.arange(1,1+first), mean, label=labelize_inv(algo), linestyle=linestyles[j%(len(linestyles))],color=colors[j%len(colors)])
...
ax[1][0].set_ylim(ymax=0.07,ymin=0.04)
...
ax[1][1].set_title(r"$H_1, \Delta \sim {U}(0.001, 0.5)$")
...
plt.tight_layout()
plt.savefig(f'results_data/simu-main.pdf', dpi=300)
